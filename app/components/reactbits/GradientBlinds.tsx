...
const GradientBlinds = ({
  ...
  // mantém o estado de pausa sem re-inicializar o renderer
  useEffect(() => {
...
`;
    ...
    // Re-medição após o layout assentar (robustez no load)
    const settleTimers = [60, 180, 400, 800, 1400].map((ms) => window.setTimeout(resize, ms));
    ...
    const loop = (t) => {
      ...
      if (pausedRef.current) return; // fora da viewport: não atualiza nem renderiza
...
export default GradientBlinds as any;
